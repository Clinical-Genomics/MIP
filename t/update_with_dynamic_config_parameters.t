...
## Modules with import
    my %perl_module =
      ( q{MIP::Active_parameter} => [qw{ update_with_dynamic_config_parameters }], );
...
diag(   q{Test update_with_dynamic_config_parameters from Active_parameter.pm}
      . $COMMA
      . $SPACE . q{Perl}
      . $SPACE
      . $PERL_VERSION
...
my %active_parameter = (
    ...
    sample_info_file =>
      catfile(qw{ cluster_constant_path! analysis_constant_path! case_id!_qc_sample_info.yaml }),
    some_array => [
    ...
    sv_vep_plugin => {
        ...
        LofTool => {
            exists_check => q{file},
            path => catfile(qw{ cluster_constant_path! analysis_constant_path! loftool.txt }),
        },
    },
);
...
is( $active_parameter{pedigree_file}, $updated_pedigree_file, q{Updated pedigree file path} );
