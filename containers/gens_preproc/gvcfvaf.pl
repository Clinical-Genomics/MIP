...
while (<GNOMAD>) {
    ...
    print STDERR "Skipping gnomad $gnomad_chr $gnomad_pos "
      . $gvcf->{chr} . " "
      . $gvcf->{start} . "\n"
    ...
    if ( $gvcf->{start} < $gnomad_pos or chr_less( $gvcf->{chr}, $gnomad_chr ) ) {
        ...
        do {
          ...
          or ( $gvcf->{end} >= $gnomad_pos
            or $gvcf->{start} >= $gnomad_pos
            or $gvcf->{chr} ne $prev_gvcf_chr );
    }
    ...
    unless ( $a[8] =~ /:AD:/ ) {
        ...
        for my $i ( 0 .. @fmt ) {
            if ( $fmt[$i] eq "GT" ) {
                ...
                $alt = $b;
                return \%data
                  if $alt != 0 and ( !defined $ALT[ $alt - 1 ] or length( $ALT[ $alt - 1 ] ) > 1 );
            }
        }
        for my $i ( 0 .. @fmt ) {
            if ( $fmt[$i] eq "AD" ) {
