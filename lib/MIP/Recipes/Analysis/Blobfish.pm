...
## MIPs lib/
use MIP::Constants qw{ $ASTERISK $AMPERSAND $COLON $DOT $LOG_NAME $NEWLINE $SPACE $UNDERSCORE };
...
##          : $temp_directory          => Temporary directory
  ...
  SAMPLE_ID:
    ...
    ## Get outdir_path
    my $outdir_path = catdir( $active_parameter_href->{outdata_dir}, $case_id, $recipe_name );
    ...
    if ( $recipe{mode} == 1 ) {
        ...
        my $de_outfile_name =
          $conditions[0] . $UNDERSCORE . q{vs} . $UNDERSCORE . $conditions[1] . q{.results.tsv};
...
##          : tx2gene_file_path => Outfile path
    ...
    $tx2gene_generator .= q? -nae 'BEGIN {print q{TXNAME,GENEID} . qq{\n}; %txgene;}?;
    ...
    $tx2gene_generator .= q? {$gene = $F[9]; $gene =~ tr/[;"]//d; $tx = $F[11]; $tx =~tr/[;"]//d;?;
    ## Store in hash
