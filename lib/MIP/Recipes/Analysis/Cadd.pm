...
BEGIN {
    ...
    our @EXPORT_OK = qw{ analysis_cadd analysis_cadd_gb_38 analysis_cadd_panel };
...
##          : $sample_info_href        => Info on samples and case hash {REF}
    ...
    my $cadd_infile_path = $view_outfile_path;

    if ( $cadd_version eq q{v1.5} ) {
        ...
        $cadd_infile_path = $synonyms_outfile_path;
    }
    ...
    if ( $cadd_version eq q{v1.5} ) {
        ...
        my $perl_outfile_path =
          $outfile_path_prefix . $UNDERSCORE . q{synonyms} . $DOT . q{tsv};
        ## Perl
        perl_nae_oneliners(
            {
                filehandle      => $filehandle,
        ...
        htslib_bgzip(
            {
                ...
                force           => 1,
                infile_path     => $perl_outfile_path,
                stdoutfile_path => $cadd_outfile_path,
                write_to_stdout => 1,
            }
        );
        say {$filehandle} $NEWLINE;
