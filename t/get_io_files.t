...
## Modules with import
    my %perl_module = ( q{MIP::File_info} => [qw{ get_io_files set_io_files }], );
...
diag(   q{Test get_io_files from File_info.pm}
      . $COMMA
      . $SPACE . q{Perl}
      . $SPACE
      . $PERL_VERSION
...
## Given a file info with file paths
my %file_info = (
    $id => {
...
## Given an order to process recipes
my @order_recipes =
...
## Given recipes, order of recipes and chains
my %parameter = (
...
## Then file paths should be inherited from base i.e MIP infiles for new recipe
is_deeply(
    \@{ $io{$stream_in}{file_paths} },
...
## Given of bwa_mem for $id and $id_2 for chain main

## When setting recipe io for $id with stream out
set_io_files(
    {
        chain_id       => $chain_main,
        id             => $id,
...
## When setting recipe io for $id_2 with stream out
set_io_files(
    {
        chain_id       => $chain_main,
        id             => $id_2,
...
## Given new recipe which is downstream of bwa_mem recipe
my $second_recipe = q{picard_mergesamfiles};
...
## Then outfile for bwa_mem should be returned as infiles for second_recipe for sample_1
is_deeply(
...
## Then outfile for bwa_mem should be returned as infiles for second_recipe for sample_2
is_deeply(
    \@{ $file_info{$id}{bwa_mem}{file_paths} },
    \@{ $io_id_2{$stream_in}{file_paths} },
...
## Then outfile for second_recipe should be returned
is_deeply(
    \@{ $file_info{$id}{picard_mergesamfiles}{file_paths} },
    \@{ $io{$stream_out}{file_paths} },
    q{Got picard_mergesamfiles outfile features from second_recipe for sample_1}
);
...
## Then outfile for second_recipe should be returned as infiles for $first_in_chain_recipe_name
is_deeply(
    \@{ $file_info{$id}{picard_mergesamfiles}{file_paths} },
    \@{ $io{$stream_in}{file_paths} },
...
## Then outfile for second_recipe should be returned as infiles for downstream_recipe
is_deeply(
    \@{ $file_info{$id}{picard_mergesamfiles}{file_paths} },
    \@{ $io{$stream_in}{file_paths} },
...
## Then outfile for second_recipe should be returned as infiles for delly_call recipe
is_deeply(
    \@{ $file_info{$id}{picard_mergesamfiles}{file_paths} },
    \@{ $io{$stream_in}{file_paths} },
