package MIP::Cli::Mip::Analyse::Rd_dna_panel;
...
command_long_description(q{Rare disease DNA analysis on panel sequence data});

command_usage(q{mip <analyse> <rd_dna_panel> <case_id> --config <config_file> });
...
sub run {
    ...
    my $level = q{rd_dna_panel};

    my %parameter = get_parameter_from_definition_files( { level => $level, } );

    my @rd_dna_panel_definition_file_paths =
    ...
    my @order_parameters = get_order_of_parameters(
        { define_parameters_files_ref => \@rd_dna_panel_definition_file_paths, } );
