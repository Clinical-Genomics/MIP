...
BEGIN {
    ...
    our @EXPORT_OK = qw(catvariants genotypegvcfs selectvariants variantrecalibrator applyrecalibration calculategenotypeposteriors combinevariants varianteval leftalignandtrimvariants);
...
##Returns  : "@commands"
##Arguments: $intervals_ref, $infile_paths_ref, $outfile_path, $referencefile_path, $stderrfile_path, $FILEHANDLE, $gatk_path, $pedigree, $dbsnp_file_path, $indel_gold_standard_file_path, $downsample_to_coverage, $gatk_disable_auto_index_and_file_lock, $logging_level, $pedigree_validation_type
##         : $intervals_ref                         => One or more genomic intervals over which to operate {REF}
##         : $infile_paths_ref                      => Infile paths
##         : $outfile_path                          => Outfile path
##         : $referencefile_path                    => Reference sequence file
...
##         : $pedigree_validation_type              => Validation strictness for pedigree
	...
	indel_gold_standard_file_path => { strict_type => 1, store => \$indel_gold_standard_file_path },
	downsample_to_coverage => { allow => qr/^\d+$/,
				    strict_type => 1, store => \$downsample_to_coverage },
	gatk_disable_auto_index_and_file_lock => { default => 0,
						   allow => [0, 1],
    ...
    ## Common gatk options
    my @commands = qw(--analysis_type VariantEval);  #stores commands depending on input parameters
	...
	push(@commands, "--dbsnp ".$dbsnp_file_path);
    }
    if ($indel_gold_standard_file_path) {

	push(@commands, "--goldStandard ".$indel_gold_standard_file_path);
    }

    ## Infile
    if (@$infile_paths_ref) {

	push(@commands, "--eval ".join("--eval ", @$infile_paths_ref));
    }
...
sub leftalignandtrimvariants {

##leftalignandtrimvariants

##Function : Perl wrapper for writing GATK leftalignandtrimvariants recipe to $FILEHANDLE. Based on GATK 3.7.0.
##Returns  : "@commands"
##Arguments: $intervals_ref, $infile_path, $outfile_path, $referencefile_path, $stderrfile_path, $FILEHANDLE, $gatk_path, $pedigree, $downsample_to_coverage, $gatk_disable_auto_index_and_file_lock, $logging_level, $pedigree_validation_type, $split_multiallelics
##         : $intervals_ref                         => One or more genomic intervals over which to operate {REF}
...
##         : $split_multiallelics                   => Split multiallelic records and left-align individual alleles
	...
	pedigree_validation_type => { default => "SILENT",
				      allow => ["STRICT", "SILENT"],
				      strict_type => 1, store => \$pedigree_validation_type },
	split_multiallelics => { default => 0,
				 allow => [0, 1],
				 strict_type => 1, store => \$split_multiallelics },
    };
    ...
    ## Common gatk options
    my @commands = qw(--analysis_type LeftAlignAndTrimVariants);  #stores commands depending on input parameters
	...
	push(@commands, "--splitMultiallelics");
    }
