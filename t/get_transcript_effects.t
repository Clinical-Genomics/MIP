...
my @transcript_effects = ($allele, $consequence_term, $transcript_id, $HGNC_ID, $hgnc_symbol);
my %vep_format_field_column = (Allele => $ALLELE_INDEX,
			 Consequence => $CONSEQUENCE_INDEX,
			 Feature => $TRANSCRIPT_INDEX,
			 HGNC_ID => $HGNC_ID_INDEX,
			 SYMBOL => $HGNC_SYMBOL_INDEX,
);
my %csq = get_transcript_effects({transcript_effects_ref => \@transcript_effects,
vep_format_field_column_href => \%vep_format_field_column});

my %expected_csq = (allele => $allele,
