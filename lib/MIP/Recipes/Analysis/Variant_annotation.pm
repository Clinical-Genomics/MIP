...
BEGIN {
    ...
    our @EXPORT_OK =
      qw{ analysis_variant_annotation analysis_variant_annotation_panel analysis_vcfanno_preop };

}

sub analysis_vcfanno_preop {

## Function: Annotate variants with vcfanno
## Returns:
...
##          : $recipe_name           => Recipe name
    ...
    my $upper_file_number = $recipe_resource{core_number} - 1;
    my @splitted_infiles  = map { $temp_outfile_path_prefix . $UNDERSCORE . $_ }
      ( '00' .. "$upper_file_number" );
    my @splitted_outfiles =
      map { $temp_outfile_path_prefix . $UNDERSCORE . $_ . $DOT . q{vcf.gz} }
      ( '00' .. "$upper_file_number" );
    my @splitted_stderrfiles = map { $recipe_info_path . $DOT . $_ . q{.stderr.txt} }
      ( '00' .. "$upper_file_number" );
    ...
    gnu_split(
        {
            ...
            numeric_suffixes => 1,
            prefix           => $temp_outfile_path_prefix . $UNDERSCORE,
        }
    );
    say {$filehandle} $NEWLINE;
  ...
  FILE_NUMBER:
    for my $file_number ( 0 .. $upper_file_number ) {
