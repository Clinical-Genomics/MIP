...
## Modules with import
    my %perl_module = (
        q{MIP::Check::Reference} => [qw{ check_bwa_prerequisites }],
...
diag(   q{Test check_bwa_prerequisites from Reference.pm v}
    ...
    check_bwa_prerequisites(
        {
            ...
            job_id_href             => \%job_id,
            log                     => $log,
            parameter_build_name    => $parameter_build_name,
...
## Given build parameters, when not set to build
$parameter{$parameter_build_name}{build_file} = 0;
...
## Then
is( $trap->stderr, $EMPTY_STR, q{Skipped building bwa_mem prerequisites} );
