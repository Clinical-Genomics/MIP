...
sub analysis_picardtools_mergesamfiles_rna {
...
##          : $temp_directory          => Temporary directory
    ...
    use MIP::Get::File qw{ get_file_suffix };
    use MIP::Get::Parameter qw{ get_module_parameters };
    use MIP::IO::Files qw{ migrate_file migrate_files };
    use MIP::Processmanagement::Slurm_processes
      qw{ slurm_submit_job_sample_id_dependency_add_to_sample };
    use MIP::Program::Alignment::Picardtools qw{ picardtools_mergesamfiles };
    use MIP::QC::Record qw{ add_program_outfile_to_sample_info };
    ...
    # Assemble infile paths by adding directory and file ending
    my @infile_paths =
      map { catfile( $temp_directory, $_ ) . $infile_tag . $infile_suffix }
      @{ $infile_lane_prefix_href->{$sample_id} };

    my $outfile_path = catfile( $temp_directory,
        $merged_infile_prefix . $outfile_tag . $outfile_suffix );
    ...
    say {$FILEHANDLE} q{## Copy file (.bam and .bai) from temporary directory};
    migrate_file(
        {
            FILEHANDLE  => $FILEHANDLE,
            infile_path => catfile(
                $temp_directory,
                $merged_infile_prefix . $outfile_tag . $ASTERIX
            ),
