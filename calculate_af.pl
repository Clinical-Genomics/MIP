...
GetOptions('cak|af_key_suffixes:s' => \@af_key_suffixes,
...
read_infile({af_key_suffixes_ref => \@af_key_suffixes,
...
##         : $calculate_af_version => Calculate af version
	...
	if ($_=~/^##(\S+)=/) { # MetaData
		...
		if ($key=~/$af_prefix\_(\w+)/) {  #Collect af prefix keys
			...
			if (any {$_ eq $af_key_suffix} @$af_key_suffixes_ref) {  #If element is part of array
		...
		foreach my $af_key_suffix (@$af_key_suffixes_ref) {
		    ...
		    ## Write vcf Header
		    say '##INFO=<ID=AF_'.$af_key_suffix.',Number=A,Type=Float,Description="Allele frequency in the '.$af_key_suffix.' populations calculated from AC and AN, in the range (0,1)">';
		}
	    }
	    else {
		...
		foreach my $key (keys %{$af_header{AN}}) {  #Either "AN" or "AC" keys will do
		    ...
		    ## Write vcf Header
		    say '##INFO=<ID=AF_'.$key.',Number=A,Type=Float,Description="Allele frequency in the '.$key.' populations calculated from AC and AN, in the range (0,1)">';
		}
	    }
		...
		if (exists($af_key{$keys[0]})) {
		    ...
		    if ( (@$af_key_suffixes_ref) && ($af_counter eq (scalar(@$af_key_suffixes_ref) * 2)) ) {
		...
		foreach my $af_key_suffix (@$af_key_suffixes_ref) {
		    ...
		    calculate_af({af_key_href => \%af_key,
				  afs_ref => \@afs,
...
##         : $af_key_suffix => Key to calcluate allele frequency for
    ...
    my $tmpl = { 
	af_key_href => { required => 1, defined => 1, default => {}, strict_type => 1, store => \$af_key_href},
	afs_ref => { required => 1, defined => 1, default => [], strict_type => 1, store => \$afs_ref},
	af_key_suffix => { required => 1, defined => 1, strict_type => 1, store => \$af_key_suffix},
    };
    
    check($tmpl, $arg_href, 1) or die qw[Could not parse arguments!];

    if( ($af_key_href->{AC}{$af_key_suffix}) && (isnum($af_key_href->{AC}{$af_key_suffix}) ) ) {  #For floats and integers

	if ($af_key_href->{AN}{$af_key_suffix} != 0) {  #Do not divide by 0

	    $af_key_href->{AF}{$af_key_suffix} = $af_key_href->{AC}{$af_key_suffix} / $af_key_href->{AN}{$af_key_suffix};
