...
## Modules with import
    my %perl_module = (
        ...
        q{MIP::Test::Fixtures} => [
            qw{ test_add_io_for_recipe test_add_io_for_recipe test_log test_mip_hashes }
        ],
    );
...
diag(   q{Test analysis_analysisrunstatus from Analysisrunstatus.pm}
      . $COMMA
      . $SPACE . q{Perl}
      . $SPACE
      . $PERL_VERSION
...
$active_parameter{config_file_analysis}             = catfile(qw{ a config file });
$active_parameter{sample_info_file}                 = q{qc_sample_info_file};
