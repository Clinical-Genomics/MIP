...
BEGIN {
    ...
    our @EXPORT_OK = qw{sambamba_view sambamba_index sambamba_sort sambamba_markdup sambamba_flagstat sambamba_depth};
}
...
##         : $mode              => Mode unit to print the statistics on
    ...
    if(@$depth_cutoffs_ref) {
		...
		push @commands, q{--cov-threshold}. $SPACE . join q{ --cov-threshold} . $SPACE, @{ $depth_cutoffs_ref };
    }
    if ($min_base_quality) {
